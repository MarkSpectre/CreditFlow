...
function Login() {
  ...
  const handleQuickDemoLogin = (roleOverride) => {
    const role = roleOverride || selectedRole;
    const isLoanOfficer = role === 'loan_officer';
    const userObj = {
      first_name: isLoanOfficer ? 'Sarah' : 'Alex',
      last_name: isLoanOfficer ? 'Jenkins (Officer)' : 'Morgan',
      email: isLoanOfficer ? 'officer@bank.com' : 'alex@company.com',
      role: role,
      onboarding_completed: true,
      picture: null
    };
    localStorage.setItem('token', `demo_access_token_${Date.now()}`);
  ...
  const handleGoogleSignIn = useGoogleLogin({
    onSuccess: async (tokenResponse) => {
      try {
        ...
        console.error("Backend Google token verification failed, using demo login:", err);
        handleQuickDemoLogin(selectedRole);
      }
    },
    onError: (err) => {
      console.warn("Google OAuth popup error or blocked by client ID config:", err);
      handleQuickDemoLogin(selectedRole);
    },
    flow: 'implicit',
  });
  ...
  const triggerGoogleSignIn = () => {
    try {
      ...
      console.warn("Google sign-in invocation failed:", err);
      handleQuickDemoLogin(selectedRole);
    }
  };
  ...
  const handleStandardLogin = async (e) => {
    ...
    try {
      ...
      const userObj = res.data.user || {
        ...
        email: username.includes('@') ? username : `${username}@CreditFlow.com`,
        role: selectedRole,
        onboarding_completed: true
      };
      localStorage.setItem('user_profile', JSON.stringify(userObj));
    ...
    <div className="bg-[#0b0f19] min-h-screen flex flex-col justify-center items-center relative overflow-hidden font-sans p-4">
      ...
      <div className="box-border bg-[#111726]/80 backdrop-blur-xl border border-slate-800 rounded-3xl w-full max-w-md flex flex-col items-center justify-center p-8 gap-y-6 shadow-2xl z-10">
        ...
        <div className="flex flex-col items-center gap-2">
          <img src={logo} alt="CreditFlow Logo" className="h-12 w-auto object-contain" />
          <h1 className="text-white text-2xl font-extrabold tracking-tight">Welcome to CreditFlow</h1>
        ...
        <div className="w-full bg-[#182032] p-1.5 rounded-2xl border border-slate-800 grid grid-cols-2 gap-1.5">
          <button
            ...
            className={`flex items-center justify-center gap-2 py-2.5 rounded-xl text-xs font-bold transition-all ${selectedRole === 'business_owner'
          ...
          <button
            ...
            className={`flex items-center justify-center gap-2 py-2.5 rounded-xl text-xs font-bold transition-all ${selectedRole === 'loan_officer'
                ? 'bg-[#26e6b6] text-slate-950 shadow-lg shadow-[#26e6b6]/20'
                : 'text-slate-400 hover:text-white hover:bg-slate-800/50'
              }`}
          >
        ...
        <form onSubmit={handleStandardLogin} className="w-full space-y-4">
          ...
          <div className="pt-2 space-y-2.5">
            ...
            <button
              ...
              className="w-full bg-slate-800/60 border border-slate-700/60 text-[#26e6b6] py-2.5 rounded-xl font-bold hover:bg-slate-800 hover:border-[#26e6b6]/40 active:scale-[0.99] transition-all text-xs flex items-center justify-center gap-2"
            >
              ⚡ Quick Demo Sign In ({selectedRole === 'business_owner' ? 'Business Owner' : 'Loan Officer'})
            </button>
          </div>
        </form>
      </div>
